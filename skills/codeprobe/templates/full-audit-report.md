<!--
  ...
  Structure:
    ...
    2. Tiered findings:
       ...
       - Conflicting Recommendations (only when detected — see SKILL.md §7A step 2)
...
<!-- Omit this section entirely when no conflicts were detected (the common case). -->

{For each pair of overlapping findings with opposing recommendations (see SKILL.md Section 7A step 2):}

**{id_a} vs {id_b}** | `{file}:{lines}`

- **{id_a}** ({category_a}, {severity_a}): {suggestion_a}
- **{id_b}** ({category_b}, {severity_b}): {suggestion_b}
- **Tradeoff:** {one line naming what the operator is choosing between}
...
*Generated by codeprobe v{version} — Read-only analysis — no files were modified*

<!-- {version} = the orchestrator SKILL.md frontmatter metadata.version -->
